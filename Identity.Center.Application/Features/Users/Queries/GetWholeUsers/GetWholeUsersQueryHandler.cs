...
internal sealed class GetWholeUsersQueryHandler(IServiceProvider provider) : IQueryHandler<GetWholeUsersQuery, IPaginatedResult<BasicUserInfoDto>>
{
  private readonly IIdentityRepository<User> _repo = provider.GetRequiredService<IIdentityRepository<User>>();

  public async Task<Result<IPaginatedResult<BasicUserInfoDto>>> Handle(GetWholeUsersQuery request, CancellationToken cancellationToken)
  {
    IPaginatedResult<BasicUserInfoDto> result = await PaginatedResult.ComputeAsync(
      _repo.Data
        ...
        .Select(row => new BasicUserInfoDto(
          ...
          row.DomainCredentials.Select(dc => new CreatedCredentialDto(
              dc.Id,
              null,
              dc.CredentialTypeId,
              dc.Username,
          ...
          row.SingleCredentials.Select(sc => new CreatedCredentialDto(
            null,
            null,
            default,
            sc.Username,
